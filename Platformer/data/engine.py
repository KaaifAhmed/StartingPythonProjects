...
class Particle(object):
    ...
    def draw(self,surface,scroll):
        ...
        if self.color is None:
            ...
            blit_center(surface,swap_color(particle_images[self.type][int(self.frame)],
                                           (255,255,255),self.color),(self.x-scroll[0],self.y-scroll[1]))
    ...
    for y_pos in range(chunk_size):
        for x_pos in range(chunk_size):
            ...
            if tar_y > 8 - height:
                ...
                tile_type = 1
            elif tar_y == 8 - height - 1:
                if random.randint(1, 6) == 1:
                    tile_type = 3
            if tile_type != 0:
                chunk_data.append([[tar_x, tar_y], tile_type])
    return chunk_data
